# AIM ---------------------------------------------------------------------
# plot the single sample analyzed objects
...
# read in the data --------------------------------------------------------
list_data.combined <- readRDS("../out/object/05_list_datasc_norm.rds")
# Idents(data.combined) <- "cell_id"

# wrangling ---------------------------------------------------------------
# check if the proportion of introonic reads is correlated to any of the other metadata in the datasets
df_meta <- list_data.combined$sparse_matrix_all@meta.data %>%
  rownames_to_column("barcode") %>%
  dplyr::select(barcode,orig.ident,nCount_RNA,nFeature_RNA,percent.mt,percent.ribo,percent.globin,prop_intronic)

saveRDS(df_meta,"../out/object/05_df_meta.rds")
...
# insert the function into wrapping and decide which panel to control
ggpairs(df_meta, columns = 3:8,
        lower = list(continuous = wrap(panelfun))) + theme_bw() + theme(strip.background = element_blank())

# make a density map of the scatter plot
# make the pariwise correlation plot
df_meta_long <- df_meta %>%
  pivot_longer(names_to = "var",values_to = "val",-c(barcode,orig.ident))

df_crossing <- crossing(var1 = df_meta_long$var %>% unique(),
         var2 = df_meta_long$var %>% unique()) %>%
  # filter out the diagonals
  filter(var1 != var2) %>%
  # filter out the combinations that are the same value but scrambled
  ...
  data.frame() %>%
  setNames("test") %>%
  separate(test,into = c("var1","var2"),sep = "-")

list_meta_full <- pmap(df_crossing, function(var1,var2){
  df1 <- df_meta %>%
    select(barcode,orig.ident,all_of(var1)) %>%
    setNames(c("barcode","orig.ident","var1_value")) %>%
    mutate(var1_name = var1)
  
  df2 <- df_meta %>%
    select(barcode,orig.ident,all_of(var2)) %>%
    setNames(c("barcode","orig.ident","var2_value")) %>%
    mutate(var2_name = var2)
  
  df_tot <- left_join(df1,df2,by = c("barcode","orig.ident"))
  
  return(df_tot)
})

list_plot <- lapply(list_meta_full, function(x){
  lab_x <- unique(x$var1_name)
  lab_y <- unique(x$var2_name)
  
  x %>%
    # filter(var1_name == "nCount_RNA",var2_name == "nFeature_RNA") %>%
    ggplot(aes(x=var1_value,y=var2_value))+
    stat_density_2d(aes(fill = after_stat(density)), geom = "raster", contour = FALSE,n = 200) +
    theme_cowplot()+
    scale_fill_viridis_c(option = "turbo")+
    # facet_wrap(var1_name~var2_name,scales = "free") +
...
# plots -------------------------------------------------------------------
# library(clustree)
# run the analysis for each individula object
# x <- list_data.combined$sparse_matrix_all

list_tree <- lapply(list_data.combined,function(x){
  # extract the name of the obkect
  name <- unique(x@meta.data$orig.ident)
  
  # make the plot
  plot <- clustree::clustree(x@meta.data[,grep("RNA_snn_res",colnames(x@meta.data))],
                     prefix = "RNA_snn_res.") +
    ggtitle(name)
  
  return(plot)
})
wrap_plots(list_tree)
ggsave("../out/plot/05_UMAPCluster_tree_V5.pdf",width = 30,height = 10)

# pick res o.2 for all the datasets
list_UMAP01 <- lapply(list_data.combined,function(x){
  # extract the name of the obkect
  name <- unique(x@meta.data$orig.ident)
  
  # make the plot
  plot <- DimPlot(x, reduction = "umap", group.by = "RNA_snn_res.0.2",label = T,raster = T) +
    ggtitle(name)
  
  return(plot)
})
wrap_plots(list_UMAP01)
ggsave("../out/plot/05_UMAPCluster_tree_V5.pdf",width = 30,height = 10)
