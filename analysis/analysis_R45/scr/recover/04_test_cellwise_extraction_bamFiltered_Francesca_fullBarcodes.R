...
lapply(files,function(file_id){
  ...
  file <- paste0("/beegfs/scratch/ric.cosr/ric.cosr/CRtest_genova_pedrini/cellranger/cellranger7/output/",
                 file_id,
                 "/outs/summarized_feature_test_all_fullBarcodes.txt")
  file
  ...
  saveRDS(sparse_matrix,paste0("../out/object/sparse_matrix_all_",file_id,"_fullBarcodes.rds"))
})
...
list_sparse_matrix <- lapply(files,function(file_id){
  # print(x)
  sparse_matrix <- readRDS(paste0("../out/object/sparse_matrix_all_",file_id,"_fullBarcodes.rds"))
