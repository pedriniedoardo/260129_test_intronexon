...
list_feature <- lapply(list_folder, function(x){
  feature_exon <- read_table(paste0(x,"feature_counts_possorted_genome_bam_exon_filterd_common_seurat.txt"),col_names = F) %>%
    dplyr::rename(count_exon = X1,gene = X2)
  
  feature_intron <- read_table(paste0(x,"feature_counts_possorted_genome_bam_intron_filterd_common_seurat.txt"),col_names = F) %>%
  ...
  feature_all <- read_table(paste0(x,"feature_counts_possorted_genome_bam_original_common_seurat.txt"),col_names = F) %>%
    dplyr::rename(count_all = X1,gene = X2)
  
  # read in the feature counts derived from the filtering process on the original bam file
  feature_all_filtered <- read_table(paste0(x,"feature_counts_possorted_genome_bam_all_filtered_common_seurat.txt"),col_names = F) %>%
...
saveRDS(list_feature,"../out/object/list_feature_seurat.rds")
...
saveRDS(list_df_tot,"../out/object/list_intronic_metrics_seurat.rds")
...
df_01 %>%
  ggplot(aes(x=count_all_adj,y=count_allFilter_adj)) +
...
ggsave("../out/plot/01_scatter_allFiltered_vs_all_seurat.pdf",width = 6,height = 6)
...
ggsave("../out/plot/01_scatter_allFiltered_vs_ExonAndIntron_seurat.pdf",width = 6,height = 6)

# plot the count_allFilter vs the exon only
df_02 %>%
  ggplot(aes(x = count_exon_adj, y = count_allFilter_adj)) +
  geom_point(shape = 1, alpha = 0.1) +
  scale_y_log10() +
  scale_x_log10() +
  theme_bw() +
  ...
  theme(strip.background = element_blank()) +
  geom_abline(slope = 1,intercept = 0,col="red",lty = "dashed")
ggsave("../out/plot/01_scatter_allFiltered_vs_Exon_seurat.pdf",width = 6,height = 6)
...
# assamble the panel
P0/P1/P2/P3/P4
ggsave("../out/plot/01_EDA_intronProp_seurat.pdf",width = 16,height = 20)
