...
data_intronic <- readRDS(paste0("../out/object/sparse_matrix_intron_BSedo_fullBarcodes.rds"))
data_all <- readRDS(paste0("../out/object/sparse_matrix_all_BSedo_fullBarcodes.rds"))
...
df_all_full <- left_join(df_all,df_intronic,by ="barcode") %>%
  ...
  dplyr::filter(!is.na(count_intronic))
