# AIM ---------------------------------------------------------------------
# test the fucntions developed to process the oputput of the snakemake pipeline.
...
source(file = "scr/000_functions_IntroExon.R")
...
# read in the data --------------------------------------------------------
# sample processing of a single sample

# read in the GTF file
gtf_file_loc <- "../../data/test_connect5k_PBMC/results/PureIntronExon/merged/genes.gtf"
GTF_obj <- ReadGTFIntronExon(gtf_file_loc)

str(GTF_obj)

# read in the sparse matrices from a single sample
list_pure_ExonIntron <- Read10XIntronExon(gtf_obj = GTF_obj,file = "../../data/test_connect5k_PBMC/results/PureIntronExon/merged/connect_5k_pbmc_NGSC3_ch1_gex_1/")

glimpse(list_pure_ExonIntron)
list_pure_ExonIntron$sparse_matrix_exon[c("ACTB","GAPDH","TUBB"),1000:1050]

# print the size
dim(list_pure_ExonIntron$sparse_matrix_exon)
format(object.size(list_pure_ExonIntron$sparse_matrix_exon), units = "Gb")

dim(list_pure_ExonIntron$sparse_matrix_intron)
format(object.size(list_pure_ExonIntron$sparse_matrix_intron), units = "Gb")

# test the timing of the Read10XIntronExon function
...
list_pure_ExonIntron_filter <- lapply(list_pure_ExonIntron, function(x){
  x[,cell_barcodes]
})

# generate the combined matrix from the pure matrices
mtx_test <- list_pure_ExonIntron_filter$sparse_matrix_intron + list_pure_ExonIntron_filter$sparse_matrix_exon

# now the two matrices should be equal
dim(mtx_test)
dim(mtx_raw)

identical(mtx_test,mtx_raw)
...
df_extracted <- Matrix::rowSums(mtx_test) %>%
  data.frame() %>%
...
df_summary_pure <- inner_join(
  Matrix::rowSums(list_pure_ExonIntron_filter$sparse_matrix_exon) %>%
    data.frame() %>%
    setNames("pure_exonic") %>%
    rownames_to_column("gene"),
  Matrix::rowSums(list_pure_ExonIntron_filter$sparse_matrix_intron) %>%
    data.frame() %>%
...
df_summary_gene <- PropIntronicPerGene(sparse_matrix_exon = list_pure_ExonIntron_filter$sparse_matrix_exon,
                                       sparse_matrix_intron = list_pure_ExonIntron_filter$sparse_matrix_intron)
...
df_summary_cell <- PropIntronicPerCell(sparse_matrix_exon = list_pure_ExonIntron_filter$sparse_matrix_exon,
                                       sparse_matrix_intron = list_pure_ExonIntron_filter$sparse_matrix_intron)
...
list_mtx_all_filtered <- list(all = mtx_raw,
                              pure_exon = list_pure_ExonIntron_filter$sparse_matrix_exon,
                              pure_intron = list_pure_ExonIntron_filter$sparse_matrix_intron)
