...
# # read in data ------------------------------------------------------------
# # read in the dataset not runnign cellbender
# test_wcellb <- readRDS("/idle/ric.cosr/ric.cosr/maurizio.aurora/bop_Tiget/CyteTypeR_annotation_TIGET_res0.5.rds")
# DimPlot(test_wcellb,label = T)
# 
# DimPlot(test_wcellb,group.by = "cytetype_annotation_RNA_snn_harmony_res.0.5",label = T)
...
# })
# 
# cov_test_02 <- c("percent_mito", "percent_ribo")
...
# ggsave("../../out/plot/000_UMAPCluster_technical_TIGET.pdf",width = 10,height = 8)
# 
# # make violin plots
# list_plot_technical <- lapply(c(cov_test_01,cov_test_02), function(x){ 
#   test <- VlnPlot(object = test_wcellb,features = x, group.by = "cytetype_cellOntologyTerm_RNA_snn_harmony_res.0.5",raster = T)
#   return(test)
# })
# 
# # make it a dataframe
...
#   )
# ggsave("../../out/plot/000_ViolinCluster_technical_TIGET.pdf",width = 9,height = 10)

# -------------------------------------------------------------------------

# read in the dataset not running cellbender
test_wocellb <- readRDS("/idle/ric.cosr/ric.cosr/maurizio.aurora/bop_Tiget/CyteTypeR_annotation_TIGET_res0.5.rds")
...
test_wocellb@meta.data %>%
  select(cytetype_annotation_RNA_snn_harmony_res.0.5,cytetype_cellOntologyTerm_RNA_snn_harmony_res.0.5,cytetype_annotation_RNA_snn_harmony_res.0.5,cytetype_cellState_RNA_snn_harmony_res.0.5,cytetype_RNA_snn_harmony_res.0.5,cytetype_ontologyTerm_RNA_snn_harmony_res.0.5) %>%
  skim()

# test_wcellb@meta.data %>%
#   group_by(cytetype_annotation_RNA_snn_harmony_res.0.5,
#            cytetype_cellOntologyTerm_RNA_snn_harmony_res.0.5,
#            cytetype_cellState_RNA_snn_harmony_res.0.5,
#            cytetype_RNA_snn_harmony_res.0.5,
#            cytetype_ontologyTerm_RNA_snn_harmony_res.0.5) %>%
#   summarise()
...
meta_test_wocellb %>%
  ...
  pivot_longer(names_to = "test",values_to = "ID",cytetype_annotation_RNA_snn_harmony_res.0.5:cytetype_ontologyTerm_RNA_snn_harmony_res.0.5) %>%
  filter(str_detect(ID,pattern = "ibro"))
...
meta_wocellb <- test_wocellb@meta.data %>%
  select(orig.ident,
         cytetype_annotation_RNA_snn_harmony_res.0.5,
         cytetype_cellOntologyTerm_RNA_snn_harmony_res.0.5,
         cytetype_cellState_RNA_snn_harmony_res.0.5,
         cytetype_RNA_snn_harmony_res.0.5,
         cytetype_ontologyTerm_RNA_snn_harmony_res.0.5) %>%
...
meta_wocellb_full <- meta_wocellb %>%
  ...
  select(barcode_pivot,sample_id_our = orig.ident,sample_id_ref,cytetype_annotation_RNA_snn_harmony_res.0.5:cytetype_ontologyTerm_RNA_snn_harmony_res.0.5)
...
test_compare %>%
  # group_by(cytetype_annotation_RNA_snn_harmony_res.0.5,cell_type) %>%
  group_by(cell_type) %>%
  summarise(n = n()) %>%
  arrange(desc(n))
...
# vec_our_annotation <- c("cytetype_annotation_RNA_snn_harmony_res.0.5",
#                         "cytetype_cellOntologyTerm_RNA_snn_harmony_res.0.5",
#                         "cytetype_cellState_RNA_snn_harmony_res.0.5",
#                         "cytetype_RNA_snn_harmony_res.0.5",
#                         "cytetype_ontologyTerm_RNA_snn_harmony_res.0.5")

vec_our_annotation <- c("cytetype_cellOntologyTerm_RNA_snn_harmony_res.0.5")

# anno <- "cytetype_annotation_RNA_snn_harmony_res.0.5"
...
wrap_plots(list_hm2)
ggsave("../../out/plot/000_test_battle_jaccard_wocb_TIGET.pdf",width = 10,height = 8)
